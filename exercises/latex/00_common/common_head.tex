...
\newcommand{\optionaltextboxInternal}[1]{
\bigskip
\begin{center}
\fbox{
...
\newcommand{\ExercisePrefixBasics}[0]{\ExercisePrefix{G}}
\newcommand{\ExercisePrefixMemory}[0]{\ExercisePrefix{S}}
\newcommand{\ExercisePrefixObjectOrientation}[0]{\ExercisePrefix{O}}
\newcommand{\ExercisePrefixAdvanced}[0]{\ExercisePrefix{F}}
\newcommand{\ExercisePrefixEmbeddedC}[0]{\ExercisePrefix{C}}
\newcommand{\ExercisePrefixElevator}[0]{\ExercisePrefix{Z}}

% Creates a a link to a sample solution project.
...
\newcommand{\cpppLinkToSampleSolution}[2]{\href{./exercises/solutions/#1/.}{\texttt{./exercises/solutions/#2}}\xspace}

% Displays a hint at the sample solution.
% #1 is the name of the folder in exercises/solutions that contains the solution (e.g., linked_list)
% #2 is similar to #1, but should be valid LaTeX code in text mode (e.g., linked\_list)
\newcommand{\cpppSolutionName}[2]{Ein Lösungsvorschlag für diese Aufgabe liegt im Ordner \cpppLinkToSampleSolution{#1}{#2}.}

% Intext insertion of toolbar icon
% #1: Name of icon file in folder exercises/latex/codeliteIcons
\newcommand{\cpppInsertToolbarIcon}[1]{\includegraphics[height=1.5ex]{codeliteIcons/#1}\xspace}
...
\newcommand{\cpppInputNoPageBreakListing}[2][] { \begin{footnotesize} \begin{minipage}{\textwidth} \lstinputlisting[#1]{#2} \end{minipage} \end{footnotesize}}
% Variant for inline-inserting code
\newcommand{\cpppNoPageBreakListing}[2][] {\begin{footnotesize} \begin{minipage}{\textwidth} \begin{lstlisting}[#1]#2 \end{lstlisting}\end{minipage} \end{footnotesize}}

% The cpppInputListing command allows a listing to continue on the next page.
% The first paramer is optional and arguments from lstinputlisting can be provided here.
% The second parameter is the path to the listing.
\newcommand{\cpppInputListing}[2][] {\begin{footnotesize} \vspace{1.5ex} \setbox0\vbox{ \lstinputlisting[#1]{#2} } \unvbox0 \end{footnotesize} }
\newcommand{\cpppListing}[2][] {\begin{footnotesize}\vspace{1.5ex} \setbox0\vbox{ \begin{lstlisting}[#1]#2 \end{lstlisting} } \unvbox0 \end{footnotesize} }
