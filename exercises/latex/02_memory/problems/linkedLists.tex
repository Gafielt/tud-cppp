...
Vergleiche nicht den Inhalt der Items, da der Vergleich auch dann funktionieren soll, wenn \lstinline{item} den Wert \lstinline{NULL} trägt, wenn der Iterator also auf kein Element zeigt.
...
Verschieben die \lstinline{#include}-Anweisungen für die Header von \lstinline{List} und \lstinline{ListItem} aus \filename{ListIterator.hpp} nach \filename{ListIterator.cpp} und füge in \filename{ListIterator.hpp} folgendes hinzu

\cpppInputListing{02_memory/problems/listings/linkedLists_listiterator_forward.hpp}

Dies sind Vorwärtsdeklarationen (\textbf{Forward Declaration}), die dem Compiler sagen, dass die Klassen existieren, aber später definiert werden. Nun kannst du problemlos \filename{ListIterator.hpp} in \filename{List.hpp} einbinden.
