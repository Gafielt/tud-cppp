...
\hints{
    ...
    \item Um zwei C-String Literale, also z.B. \lstinline{const char * str = "Name: "}, an einander anzuhängen, muss zuerst aus einem der beiden ein anonymes \lstinline{std::string} Object erzeugt werden, welches dann über den '+'-Operator mit dem anderen konkateniert werden kann: \lstinline{std::string text = std::string(str) + "Maike";}. 
	
	Dies funktioniert, da die Klasse \lstinline{std::string} den '+'-Operator dahin gehend überladen hat, sowohl mit C-Strings als auch mit \lstinline{std::string} Objekten arbeiten zu können. Ein C-String ist dazu alleine nicht in der Lage.  
}
...
\hints{
    \item Dadurch dass \lstinline{Person} als Zeiger übergeben wird, können auch Unterklassen von \lstinline{Person}, wie z.B. \lstinline{Student}, übergeben werden. 
}
