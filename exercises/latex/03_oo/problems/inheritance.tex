...
\hints{
    ...
    \item Um zwei C-String Literale, also zB. \lstinline{const char * str = "Name: "}, an einander anzuhängen, musst du zuerst aus einem der beiden ein \lstinline{std::string} Object erzeugen, welches dann über den '+'-Operator mit dem anderen konkateniert werden kann: \lstinline{std::string text = std::string(str) + "Maike";}. 
	
	Dies funktioniert, da die Klasse \lstinline{std::string} den '+'-Operator dahin gehend überladen hat, sowohl mit C-Strings als auch mit anderen \lstinline{std::string} Objekten arbeiten zu können. Ein C-String ist dazu alleine nicht in der Lage.  
}
